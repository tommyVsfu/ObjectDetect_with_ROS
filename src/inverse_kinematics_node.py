...
class InverseKinematicsNode(object):
    def __init__(self):
        ...
        self.sub_car_cmd = rospy.Subscriber("/simple_stop_controller_node/car_cmd", Twist2DStamped, self.car_cmd_callback)
    ...
    def readParamFromFile(self):
        ...
        if not os.path.isfile(fname):
            rospy.logwarn("[%s] %s does not exist. Using alphaduck.yaml." %(self.node_name,fname))
            fname = self.getFilePath("alphaduck")
    ...
    def car_cmd_callback(self, msg_car_cmd):
        rospy.loginfo("car_cmd call back is succeed")
