...
class CamInfoReader(object):
    def __init__(self):
        ...
        self.cali_file_name = self.setupParam("~cali_file_name","alphaduck")

        # Setup publisher
        self.pub_camera_info = rospy.Publisher("/ground_projection/camera_info",CameraInfo,queue_size=1)
        ...
        self.sub_img_compressed = rospy.Subscriber("/camera/image/compressed",CompressedImage,self.cbCompressedImage,queue_size=1)
    
    def cbCompressedImage(self,msg):
        if self.camera_info_msg is not None:
	    rospy.loginfo("camera info is publishing")
